...
NINTENDO_LOGO = bytes.fromhex(
    "CEED6666CC0D000B03730083000C000D000D000D000D000D000D000D000D000D"
...
def main() -> int:
    ...
    if data[0x104:0x134] != NINTENDO_LOGO[:0x30]:
    ...
    title = data[0x134:0x144].split(b"\0", 1)[0].decode("ascii", errors="replace")
    report = "\n".join(
        [
            f"ROM: {args.rom.name}",
            f"Title: {title}",
            f"Size: {len(data)} bytes ({len(data) // 0x4000} banks)",
            f"Cartridge type: 0x{data[0x147]:02X}",
            f"ROM size code: 0x{data[0x148]:02X}",
            f"RAM size code: 0x{data[0x149]:02X}",
